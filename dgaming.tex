...
{\bfseries Premier}: An Exchangeable ERC20 token.
...
{\bfseries Reward}: An Exchangeable ERC721 token.

\begin{gather}
  S_{reward} = \{B, S, H, A\}
  \intertext{Where:}
  \begin{tabular}{c    l}
    B: & \text{Buy Reward from Exchange}\\
    S: & \text{Sell Reward to Exchange}\\
    H: & \text{Hold Reward and do noting}\\
    A: & \text{Auction Reward for Premier}\\
\end{tabular}
\end{gather}
